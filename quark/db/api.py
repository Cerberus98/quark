...
def network_find(context, limit=None, sorts=None, marker=None,
    ...
    if "shared" in filters:
        defaults = STRATEGY.get_provider_networks(context)
...
@scoped
def subnet_find(context, limit=None, sorts=None, marker=None,
                page_reverse=False, fields=None, **filters):
    ids = []
    defaults = []
    provider_query = False
    if "id" in filters:
        ids, defaults = STRATEGY.split_subnet_ids(context, filters["id"])
        if not ids and defaults and "shared" not in filters:
            provider_query = True
        if ids:
            filters["id"] = ids
    ...
    if "shared" in filters:
        defaults = STRATEGY.get_provider_subnets(context)
        if True in filters["shared"]:
            if ids:
            ...
            defaults.insert(0, INVERT_DEFAULTS)
        filters.pop("shared")
    return _subnet_find(context, limit, sorts, marker, page_reverse, fields,
                        defaults=defaults, provider_query=provider_query,
                        **filters)
